...
class _CruiseLevelScreenState extends State<CruiseLevelScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      if (userId != null) {
        ...
        int completed = 0, canceled = 0;
        for (final t in trips) {
          if (t['status'] == 'completed') completed++;
          if (t['status'] == 'canceled') canceled++;
        }
        _totalTrips = trips.length;
        _satisfactionRate = _totalTrips > 0 ? (completed / _totalTrips * 100) : 0;
        _cancellationRate = _totalTrips > 0 ? (canceled / _totalTrips * 100) : 0;
        _acceptanceRate = _totalTrips > 0 ? 100 : 0;
        _onTimeRate = completed > 0 ? 97 : 0;
        _points = completed * 10;

        // Determine tier
        if (_satisfactionRate >= 95 && _acceptanceRate >= 85 && _cancellationRate <= 2 && _onTimeRate >= 90 && _points >= 5000) {
          _currentTierIndex = 3;
        } else if (_satisfactionRate >= 90 && _acceptanceRate >= 50 && _cancellationRate <= 5 && _onTimeRate >= 80 && _points >= 2000) {
          _currentTierIndex = 2;
        } else if (_satisfactionRate >= 85 && _acceptanceRate >= 30 && _cancellationRate <= 8 && _onTimeRate >= 70 && _points >= 500) {
          _currentTierIndex = 1;
        } else {
          _currentTierIndex = 0;
        }
  ...
  Widget _buildCurrentTierCard() {
    final tier = _tiers[_currentTierIndex];
    final nextTier = _currentTierIndex < 3 ? _tiers[_currentTierIndex + 1] : null;
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [tier.color.withValues(alpha: 0.25), tier.color.withValues(alpha: 0.05)],
      ...
      child: Column(
        children: [
          ...
          const SizedBox(height: 14),
          Text(tier.name,
              style: TextStyle(color: tier.color, fontSize: 26,
            ...
            Column(
              children: [
                Row(
                  ...
                  children: [
                    Text(tier.name,
                        style: TextStyle(color: tier.color, fontSize: 12, fontWeight: FontWeight.w700)),
                    Text(nextTier.name,
                        style: TextStyle(color: nextTier.color, fontSize: 12, fontWeight: FontWeight.w700)),
                  ],
                ),
                ...
                const SizedBox(height: 6),
                Text('${nextTier.pointsRequired - _points} points to ${nextTier.name}',
  ...
  Widget _buildProgressSection() {
    final nextTier = _currentTierIndex < 3 ? _tiers[_currentTierIndex + 1] : _tiers[3];
    
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Requirements for ${nextTier.name}',
  ...
  Widget _requirementRow(String label, String current, String target, bool met) {
    return Container(
        ...
        children: [
          Container(
            ...
            child: Icon(
              ...
              color: met ? const Color(0xFF4CAF50) : Colors.white.withValues(alpha: 0.3),
              size: 18,
            ),
          ),
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: isCurrent ? Border.all(color: tier.color.withValues(alpha: 0.3)) : null,
        ),
        child: Row(
          children: [
            Container(
              ...
              child: Icon(tier.icon, color: isLocked ? tier.color.withValues(alpha: 0.4) : tier.color, size: 26),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      ...
                      if (isCurrent)
                        Container(
                          ...
                          child: Text('Current',
                              style: TextStyle(color: tier.color, fontSize: 10,
                                  fontWeight: FontWeight.w700)),
                        ),
                      if (isLocked)
                  ...
                  const SizedBox(height: 4),
                  Text('${tier.pointsRequired} points · ${tier.rewards.length} rewards',
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            Text(tier.name,
                style: TextStyle(color: tier.color, fontSize: 24,
                    fontWeight: FontWeight.w900)),
            if (isCurrent) ...[
              const SizedBox(height: 4),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    _reqLine('Acceptance ≥ ${tier.minAcceptance}%'),
                    _reqLine('Cancellation ≤ ${tier.maxCancellation}%'),
                    _reqLine('Satisfaction ≥ ${tier.minSatisfaction}%'),
                    _reqLine('On-time ≥ ${tier.minOnTime}%'),
                    _reqLine('${tier.pointsRequired} points'),
                  ],
                ),
              ),
              const SizedBox(height: 16),
                      ...
                      child: Row(
                        children: [
                          Container(
                            ...
                            child: Icon(Icons.check_rounded, color: tier.color, size: 16),
                          ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: tier.color, foregroundColor: Colors.black,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14)),
                ),
                child: Text(isCurrent ? 'Got it' : (isLocked ? 'Keep going!' : 'View rewards'),
  ...
  Widget _reqLine(String text) {
    return Padding(
      ...
      child: Row(
        children: [
          Icon(Icons.circle, color: Colors.white.withValues(alpha: 0.2), size: 6),
          const SizedBox(width: 8),
          Text(text, style: TextStyle(color: Colors.white.withValues(alpha: 0.6), fontSize: 13)),
        ],
      ),
    );
  }
